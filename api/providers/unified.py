"""
Miruro-only scraper - uses the Miruro Native API.
...
class UnifiedScraper:
    """
    Miruro-only scraper using the Miruro Native API.
    ...
    async def get_anime_info(self, anime_id: str) -> dict:
        ...
        # Try to resolve slug to AniList ID using cache
        try:
            ...
            if ids and ids.get("anilist_id"):
                ...
                try:
                    ...
                    if result and result.get("title"):
                        logger.debug(
                            f"[UnifiedScraper] AnimeInfo (Miruro, resolved anilistId={anilist_id}): OK"
                        )
                        return result
                except Exception as e:
                    logger.warning(
    ...
    async def get_episodes(self, anime_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if ids and ids.get("anilist_id"):
                ...
                try:
                    result = await self.miruro.get_episodes(anilist_id)
                    if result and result.get("episodes"):
                        logger.debug(
                            f"[UnifiedScraper] Episodes (Miruro, resolved {anilist_id}): {len(result.get('episodes', []))} eps"
                        )
                        return result
                except Exception as e:
                    logger.warning(
                        f"[UnifiedScraper] Episodes Miruro failed for resolved {anilist_id}: {e}"
                    )
        except Exception:
            pass
    ...
    async def episodes(self, anime_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if ids and ids.get("anilist_id"):
                anilist_id = str(ids["anilist_id"])
                try:
                    result = await self.miruro.episodes(anilist_id)
                    if result and result.get("episodes"):
                        return result
                except Exception:
                    pass
