...
# Lightweight watch-party state. Rooms are intentionally ephemeral and cleaned up
# automatically so the feature works without adding websocket/db infrastructure.
WATCH_TOGETHER_ROOMS = {}
WATCH_TOGETHER_LOCK = Lock()
WATCH_TOGETHER_ROOM_TTL = 6 * 60 * 60
WATCH_TOGETHER_MEMBER_TTL = 90
WATCH_TOGETHER_ROOM_ALPHABET = "ABCDEFGHJKLMNPQRSTUVWXYZ23456789"
...
def _watch_together_clean_room_id(room_id):
    return re.sub(r"[^A-Z0-9]", "", str(room_id or "").upper())[:12]


def _watch_together_clean_anime_id(anime_id):
    return str(anime_id or "").split("?", 1)[0].strip()


def _watch_together_clean_client_id(client_id=None):
...
def _watch_together_clean_name(data):
    username = session.get("username")
    raw = username or (data or {}).get("display_name") or (data or {}).get("name")
...
def _watch_together_float(value, default=0.0, upper_bound=None):
    try:
        parsed = float(value)
...
def _watch_together_bool(value, default=True):
...
def _watch_together_new_room_id():
    while True:
        room_id = "".join(
            secrets.choice(WATCH_TOGETHER_ROOM_ALPHABET) for _ in range(8)
        )
        if room_id not in WATCH_TOGETHER_ROOMS:
            return room_id


def _watch_together_touch_member(room, client_id, name, now):
    members = room.setdefault("members", {})
    existing = members.get(client_id, {})
    members[client_id] = {
        "id": client_id,
        "name": name or existing.get("name") or "Guest",
...
def _watch_together_cleanup(now=None):
    now = now or time.time()
    for room_id, room in list(WATCH_TOGETHER_ROOMS.items()):
        for member_id, member in list(room.get("members", {}).items()):
            if now - member.get("last_seen", 0) > WATCH_TOGETHER_MEMBER_TTL:
                room["members"].pop(member_id, None)

        members = room.get("members", {})
        if room.get("host_id") not in members and members:
            room["host_id"] = sorted(
                members.values(), key=lambda member: member.get("joined_at", now)
            )[0]["id"]

        room_age = now - room.get("updated_at", room.get("created_at", now))
        if room_age > WATCH_TOGETHER_ROOM_TTL or (
            not members and room_age > WATCH_TOGETHER_MEMBER_TTL
        ):
            WATCH_TOGETHER_ROOMS.pop(room_id, None)


def _watch_together_room_payload(room, client_id=None):
    now = time.time()
    members = []
    for member in room.get("members", {}).values():
        member_id = member.get("id")
        members.append(
            {
                "id": member_id,
                "name": member.get("name") or "Guest",
                "is_host": member_id == room.get("host_id"),
                "is_self": member_id == client_id,
                "last_seen": member.get("last_seen"),
            }
        )
    members.sort(key=lambda member: (not member["is_host"], member["name"].lower()))
...
def _watch_together_playback_from_payload(data, now, client_id, seq, event=None):
    duration = _watch_together_float(data.get("duration"), 0.0)
    current_time = _watch_together_float(
        data.get("current_time"), 0.0, duration if duration > 0 else None
    )
    playback_rate = _watch_together_float(data.get("playback_rate"), 1.0)
    if playback_rate <= 0 or playback_rate > 4:
        playback_rate = 1.0

    clean_event = re.sub(r"[^a-z_-]", "", str(event or data.get("event") or "sync"))[
        :24
    ]
    return {
        "current_time": current_time,
        "duration": duration,
        "paused": _watch_together_bool(data.get("paused"), True),
        "playback_rate": playback_rate,
        "event": clean_event or "sync",
        "seq": seq,
        "updated_at": now,
...
@watch_routes_bp.route("/api/watch-together/rooms", methods=["POST"])
def create_watch_together_room():
    """Create an ephemeral watch-together room for the current episode."""
    data = request.get_json(silent=True) or {}
    anime_id = _watch_together_clean_anime_id(data.get("anime_id"))
    episode_number = data.get("episode_number")

    if not anime_id or episode_number is None:
        return jsonify({"success": False, "error": "Missing anime_id or episode_number"}), 400

    client_id = _watch_together_clean_client_id(data.get("client_id"))
    name = _watch_together_clean_name(data)
    now = time.time()

    with WATCH_TOGETHER_LOCK:
        _watch_together_cleanup(now)
        room_id = _watch_together_new_room_id()
        room = {
            "id": room_id,
            "anime_id": anime_id,
        ...
        _watch_together_touch_member(room, client_id, name, now)
        room["playback"] = _watch_together_playback_from_payload(
            data, now, client_id, 1, "created"
        )
        WATCH_TOGETHER_ROOMS[room_id] = room
        payload = _watch_together_room_payload(room, client_id)

    room_url = (
        request.host_url.rstrip("/")
        + _build_clean_url(anime_id, episode_number)
        + f"?room={room_id}"
    )
    return jsonify(
        {
...
@watch_routes_bp.route("/api/watch-together/rooms/<room_id>/join", methods=["POST"])
def join_watch_together_room(room_id):
    """Join an existing watch-together room."""
    data = request.get_json(silent=True) or {}
    clean_room_id = _watch_together_clean_room_id(room_id)
    client_id = _watch_together_clean_client_id(data.get("client_id"))
    name = _watch_together_clean_name(data)
    now = time.time()
    ...
    with WATCH_TOGETHER_LOCK:
        _watch_together_cleanup(now)
        room = WATCH_TOGETHER_ROOMS.get(clean_room_id)
        if not room:
            return jsonify({"success": False, "error": "Room not found"}), 404

        anime_id = _watch_together_clean_anime_id(data.get("anime_id"))
        episode_number = data.get("episode_number")
        if anime_id and anime_id != room.get("anime_id"):
            return jsonify(
                {
                    "success": False,
                    "error": "Room is for another anime",
                    "room": _watch_together_room_payload(room, client_id),
                }
            ), 409
        if episode_number is not None and str(episode_number) != str(
            room.get("episode_number")
        ):
            return jsonify(
                {
                    "success": False,
                    "error": "Room is for another episode",
                    "room": _watch_together_room_payload(room, client_id),
                }
            ), 409
    ...
    return jsonify({"success": True, "client_id": client_id, "room": payload})


@watch_routes_bp.route("/api/watch-together/rooms/<room_id>/state", methods=["GET", "POST"])
def watch_together_room_state(room_id):
    """Read or update synchronized playback state for a room."""
    clean_room_id = _watch_together_clean_room_id(room_id)
    now = time.time()

    if request.method == "GET":
        client_id = _watch_together_clean_client_id(request.args.get("client_id"))
        with WATCH_TOGETHER_LOCK:
            _watch_together_cleanup(now)
            room = WATCH_TOGETHER_ROOMS.get(clean_room_id)
            if not room:
                return jsonify({"success": False, "error": "Room not found"}), 404
            member = room.get("members", {}).get(client_id)
            if member:
    ...
    with WATCH_TOGETHER_LOCK:
        ...
        episode_number = data.get("episode_number")
        if anime_id and anime_id != room.get("anime_id"):
            return jsonify({"success": False, "error": "Room is for another anime"}), 409
        if episode_number is not None and str(episode_number) != str(
            room.get("episode_number")
        ):
            return jsonify({"success": False, "error": "Room is for another episode"}), 409

        _watch_together_touch_member(room, client_id, name, now)
        previous_seq = int(room.get("playback", {}).get("seq", 0))
        room["playback"] = _watch_together_playback_from_payload(
            data, now, client_id, previous_seq + 1
        )
        room["updated_at"] = now
        payload = _watch_together_room_payload(room, client_id)
...
@watch_routes_bp.route("/api/watch-together/rooms/<room_id>/leave", methods=["POST"])
def leave_watch_together_room(room_id):
    """Remove a viewer from a room when the browser tab leaves."""
    data = request.get_json(silent=True) or {}
    clean_room_id = _watch_together_clean_room_id(room_id)
    client_id = _watch_together_clean_client_id(data.get("client_id"))
    now = time.time()
    ...
    with WATCH_TOGETHER_LOCK:
        ...
        room.get("members", {}).pop(client_id, None)
        room["updated_at"] = now
        _watch_together_cleanup(now)
        payload = (
            _watch_together_room_payload(room, client_id)
            if clean_room_id in WATCH_TOGETHER_ROOMS
            else None
        )

    return jsonify({"success": True, "room": payload})
